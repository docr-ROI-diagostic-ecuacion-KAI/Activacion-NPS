...
function App() {
  ...
  return <div className="app-shell"><header className="topbar"><a className="brand" href={docRoiHomeUrl} aria-label="Ir al Botiquín DOC ROI"><img src={logoUrl} alt="DOC ROI" /></a><nav style={navStyle} aria-label="Navegación principal DOC ROI"><button type="button" style={linkStyle} onClick={() => goToSection('bienvenida')}>Método</button><button type="button" style={linkStyle} onClick={() => goToSection('n8n')}>Recursos</button><button type="button" style={ctaStyle} onClick={() => goToSection('encuesta')}>Abrir herramienta</button></nav></header><main className="layout"><nav className="tabs">{tabs.map((tab, i) => <button key={tab} className={active === tabIds[i] ? 'active' : ''} onClick={() => setActive(tabIds[i])}>{tab}</button>)}</nav><section className="content" id="doc-roi-content"><Page id={active} /></section></main></div>;
}
